...
# --- Database Configuration (Matches your notebook) ---
ROUTING_DB = "routing.duckdb"

def get_routing_conn():
    return duckdb.connect(ROUTING_DB)

# --- UI Specific DB Helper Functions ---
def authenticate_user(user_id, password):
    """Authenticates user. Only shows error for incorrect password, not invalid user."""
...
with gr.Blocks(title="Bank Routing Portal", css=css) as app:
    ...
    # 2. Table Selection Action (Clicking complaint_id in dataframe)
    ...
    def handle_submit(comp_id, routed_team, escalate, escalate_to, action, routing_reason, team_email, user_type):
        ...
        df = fetch_requests_list(user_type)
        # Navigate back to list page on successful update

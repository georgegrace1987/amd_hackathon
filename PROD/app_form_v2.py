...
def launch_login_form() -> str:
    """Launch login_form.py as a separate process and return a status message."""
    try:
        # Spawns a new Python process for the login form
        subprocess.Popen([sys.executable, "login_form.py"])
        return "✅ Bank Employee Login Page launched in a new window (Port 10002)."
    except Exception as e:
        return f"❌ Failed to launch login_form.py: {str(e)}"
...
def build_ui() -> gr.Blocks:
    ...
    with gr.Blocks(title="BFSI Complaint Registration") as demo:
        ...
        with gr.Row():
            ...
            login_status   = gr.Textbox(label="Launch Status", interactive=False, visible=False)

        bank_login_btn.click(fn=launch_login_form, outputs=login_status)

...
def check_complaint_status(complaint_id: str = None, customer_id: str = None) -> list[dict]:
    """
    Customer-facing status lookup. Search by complaint_id OR customer_id
    (either may be blank, but at least one must be provided).

    Returns a list of dicts (one per matching complaint, most recent first):
        complaint_id, status, routed_team, sla_deadline
    Pulls status from complaints table and routed_team/sla info from
    routing_decisions if the complaint has been routed yet.
    """
    complaint_id = (complaint_id or "").strip()
    customer_id  = (customer_id or "").strip()
    if not complaint_id and not customer_id:
        return []
    ...
    try:
        if complaint_id:
            rows = conn.execute("""
                SELECT complaint_id, customer_id, status, created_at, sla_deadline
                FROM complaints
                WHERE complaint_id = ?
    ...
    r_conn = duckdb.connect(ROUTING_DB, read_only=True)
    results = []
    try:
        for cid, custid, status, created_at, sla_deadline in rows:
            route = r_conn.execute("""
                SELECT routed_team, routing_status, sla_hours, routed_at
            ...
            if route:
                routed_team, routing_status, sla_hours, routed_at = route
                sla_display = f"{sla_hours} hrs from routing ({routed_at})" if sla_hours else "Not set"
                display_status = routing_status or status
            else:
                routed_team    = "Not yet assigned"
                sla_display    = str(sla_deadline) if sla_deadline else "Pending classification"
                display_status = status or "New"
...
def check_duplicate_complaint(
    ...
    """
    Block submission only if the SAME customer submits a complaint that is
    identical across every field: complaint text, channel, branch,
    account_type, and category. Different complaints (even with minor
    metadata differences) from the same customer are always allowed.

    Returns the existing complaint dict if a full match is found,
    otherwise None (safe to insert).
    """
    conn = duckdb.connect(DB_PATH, read_only=True)
    try:
        row = conn.execute("""
            SELECT complaint_id, complaint, status, created_at
            FROM complaints
            WHERE customer_id = ?
              AND lower(trim(complaint)) = lower(trim(?))
              AND coalesce(lower(trim(channel)), '')      = coalesce(lower(trim(?)), '')
              AND coalesce(lower(trim(branch)), '')       = coalesce(lower(trim(?)), '')
              AND coalesce(lower(trim(account_type)), '') = coalesce(lower(trim(?)), '')
              AND coalesce(lower(trim(category)), '')     = coalesce(lower(trim(?)), '')
            LIMIT 1
        """, [
            customer_id.strip(), complaint_text.strip(),
            channel, branch, account_type, category,
        ]).fetchone()
        if row:
            return {
                "complaint_id": row[0], "complaint": row[1],

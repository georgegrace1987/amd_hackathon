...
def is_port_in_use(port: int) -> bool:
    """Check if a port is already in use (i.e., the app is already running)."""
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        s.settimeout(1)
        return s.connect_ex(('localhost', port)) == 0


def launch_app_form() -> str:
    """Launch app_form.py as a separate process and return a clickable link."""
    port = 10000
    url = f"http://localhost:{port}"
    
    if is_port_in_use(port):
        return f"✅ App is already running. [Click here to open Bank Employee Login Page]({url})"
    
    try:
        # Spawn app_form.py as a separate process
        subprocess.Popen([sys.executable, "app_form.py"])
        return f"⏳ App is starting on port {port}... Please wait a moment, then [Click here to open Bank Employee Login Page]({url})"
    except Exception as e:
        return f"❌ Failed to launch app_form.py: {str(e)}"
...
def build_ui() -> gr.Blocks:
    ...
    with gr.Blocks(title="BFSI Complaint Registration") as demo:
        ...
        with gr.Row():
            bank_login_btn = gr.Button("🏦 Bank Employee Login Page", variant="secondary")
            login_status   = gr.Markdown("")

        bank_login_btn.click(fn=launch_app_form, outputs=login_status)

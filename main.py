...
Usage:
    python main.py --mode ui          # launch customer intake form
    ...
    python main.py --mode pipeline    # generate → classify → route end-to-end
...
Options:
    ...
    --port    N        Gradio UI port (default: 10001)
...
def run_ui(port: int = 10001, share: bool = True):
...
def run_pipeline(count: int = 80, json_path: str = None):
    """Full end-to-end: generate → classify → route."""
...
def parse_args():
    ...
    parser.add_argument(
        "--mode", required=True,
        choices=["ui", "generate", "classify", "route", "pipeline", "initdb"],
    ...
    parser.add_argument("--port",   type=int,  default=10001,  help="Gradio UI port")
